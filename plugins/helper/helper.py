...
class Helper:
    ...
    async def estimate_message(self, image):
        ...
        file_id = await self.client.send_photo(
            chat_id=self.msg.chat.id, photo=img_byte_arr
        )

        file_link = (
            f"tg://openmessage?user_id={self.msg.from_user.id}&message_id={file_id}"
        )
    ...
    async def pesan_langganan(self):
        ...
        markup = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("Menfess Base 1", url=link_1),
                    InlineKeyboardButton("Memfess Base 2", url=link_2),
                ],
                [
                    InlineKeyboardButton(
                        "Coba lagi", url=f"https://t.me/{self.bot.username}?start=start"
                    )
                ],
            ]
        )
    ...
    async def daftar_pelanggan(self):
        ...
        if self.user_id == config.id_admin:
            status = "owner"
            coin = f"_{str(self.user_id)}"
    ...
    def get_time(self):
        ...
        full_time = (
            f"{hari[int(waktu[0])]}, {waktu[1]} {bulan[waktu[2]]} {waktu[3]} {waktu[4]}"
        )
        return Waktu(
            {
