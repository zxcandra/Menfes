...
async def tambah_admin_handler(client: Client, msg: types.Message):
    ...
    if target in db.get_data_bot(client.id_bot).ban:
        return await msg.reply_text(
            text=f"<i><a href='tg://openmessage?user_id={str(target)}'>User</a> sedang dalam kondisi banned</i>\n• Tidak dapat menjadikan user admin",
            quote=True,
            parse_mode=enums.ParseMode.HTML,
        )
    ...
    if member.status in status:
        return await msg.reply_text(
            text=f"❌<i>Terjadi kesalahan, <a href='tg://openmessage?user_id={str(target)}'>user</a> adalah seorang {member.status.upper()}</i>",
            quote=True,
            parse_mode=enums.ParseMode.HTML,
        )
    try:
